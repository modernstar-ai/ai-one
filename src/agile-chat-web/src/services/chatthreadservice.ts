...
export async function addExtensionToChatThread(
...
export async function removeExtensionFromChatThread(
  data: ExtensionUpdate
): Promise<ChatThread | null> {
  const apiUrl = getApiUrl(
    `/${data.chatThreadId}/extensions/${data.extensionId}`
  );
  try {
...
export async function GetChatThreadMessages(
  ...
  if (currentAssistant) {
    initialMessages = GetSystemAndWelcomeMessages(
      username,
      currentAssistant,
      chatThreadId
    );
    console.log("Chat - set initialMessages from current assistant:",
      initialMessages
    );
  }
