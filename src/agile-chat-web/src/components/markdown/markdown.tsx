...
interface Props {
  content: string;
  onCitationClick: (previousState: any, formData: FormData) => Promise<JSX.Element>;
}
...
export const Markdown: FC<Props> = (props) => {
  ...
  const CitationWrapper: FC<any> = (citationProps) => {
    return <Citation items={[{ name: citationProps.filename, id: citationProps.filename, url: citationProps.url }]} />;
  };
    ...
    <MarkdownProvider onCitationClick={props.onCitationClick}>
      <div className="">
        {Markdoc.renderers.react(content, React, {
          components: { Citation: CitationWrapper, Paragraph, CodeBlock },
        })}
      </div>
    </MarkdownProvider>
  );
